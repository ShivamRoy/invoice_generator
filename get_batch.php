...
if (isset($_GET['sedi'])) {
    ...
    // $new_invoice_number = "INVOICE #2024-AMB-0229 (Test)";

    // Insert the new invoice number into the database
    $stmt = $mysqli->prepare("INSERT INTO invoices_data (invoice_number, sedi, created_at, updated_at) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $new_invoice_number, $sedi, $created_at, $updated_at);
    $stmt->execute();
...
elseif((isset($_GET['sedi'])) && (isset($_GET['fund']))){
    ...
    // $new_invoice_number = "INVOICE #2024-AMB-0229";
    ...
    // Fetch data related to the selected SEDI
    $query = "SELECT * FROM make_generator WHERE `SEDI` = '$sedi' AND 'Funding Resource' = '$fund'";
    $query_inv_res = getData($mysqli, $query, "array");
    $query_inv_row = count($query_inv_res);
    // $invoice_num = 'INVOICE #2024-AMB-0219';
